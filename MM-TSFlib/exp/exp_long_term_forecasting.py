...
class CrossAttention(nn.Module):
    def __init__(self, query_input_dim, key_input_dim, seq_len, attention_dim=32, num_heads=4, dropout=0.1):
        super().__init__()
        self.layer_norm_q = nn.LayerNorm(query_input_dim)
        self.layer_norm_k = nn.LayerNorm(key_input_dim)

        self.proj_q = nn.Linear(query_input_dim, attention_dim)
        self.proj_k = nn.Linear(key_input_dim, attention_dim)
        self.proj_v = nn.Linear(key_input_dim, attention_dim)
        
        self.attention = nn.MultiheadAttention(embed_dim=attention_dim, num_heads=num_heads, batch_first=True, dropout=dropout)
        
        self.proj_out = nn.Linear(attention_dim, query_input_dim)
    ...
    def forward(self, x_query, x_key_value, original_query):
        ...
        out = self.proj_out(attn_out) # [Batch, Seq_Len, 1]
        out = self.dropout(out)

        alpha = torch.sigmoid(self.gate_param)
        
        final_output = (1 - alpha) * original_query + alpha * out
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        if mlp_sizes is not None:
            self.mlp = MLP(mlp_sizes,dropout_rate=0.5)
        else:
            self.mlp = None
